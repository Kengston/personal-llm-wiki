...
> Реализация — **TypeScript** (Node 22, strict ESM), порт с Python ([ADR-0012](../docs/adr/0012-language-typescript-port.md), зеркалит house-style `abcage-mcp-hub`): Fastify + pino + zod + `node:sqlite` + dotenv-flow. Исходники под `src/bridge/`, запускается собранный `dist/bridge/main.js` (`pnpm build`).

| Файл | Что делает |
|---|---|
| [`src/bridge/app.ts`](../src/bridge/app.ts) | **Fastify**-приложение: `POST /telegram/webhook/{nonce}` (3 слоя безопасности) + `GET /health`. Сборка зависимостей и жизненный цикл. |
