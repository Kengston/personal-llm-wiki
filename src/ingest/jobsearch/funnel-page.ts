 ...
 *
 *   2. РАЗРЕЗ ПО `platform` И ОТСЕЧЕНИЕ ПО ПЛОЩАДКЕ ТЕПЕРЬ СЧИТАЕТ `computeFunnel`
 ...
 *      обязан вызывающий (сейчас — `scripts/funnel-page.mjs`, который её не читает и
 *      передаёт только `asOf`/`ghostAfterDays`/`connectedSources`); пока он не читает
 *      реестр, разрез «По площадке» на реальной странице посчитан без отсечения. Это не
 *      придуманная оговорка — раздел «Ограничения» ниже называет её прямо.
 *
...
- Точка отсечения площадки (${c('since')}, [PRD] US 27: у hh — 2026-09-01) применяется в
  разрезе «По площадке» ниже, ТОЛЬКО если вызывающий передал карту ${c('platformCutoffs')}
  в ${c('computeFunnel')} ([ADR-0033]) — сама эта функция карту из реестра не строит, она
  получает уже готовый отчёт. Если её никто не передал, разрез посчитан по всей истории
  площадки, включая отклики hh до отсечения.
