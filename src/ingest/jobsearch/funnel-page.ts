 ...
 *
 *   2. РАЗРЕЗ ПО `platform` И ОТСЕЧЕНИЕ ПО ПЛОЩАДКЕ СЧИТАЕТ `computeFunnel`
 *      ([ADR-0033], [PRD] US 27/29/30) — ЭТОТ МОДУЛЬ ТОЛЬКО РЕНДЕРИТ ГОТОВЫЙ
 *      `report.breakdowns.platform`, карту отсечения (`platform → since`) сам не строит:
 *      он получает уже посчитанный `FunnelReport`, а не леджеры и не реестр площадок.
 *      Собрать карту из реестра и передать её в `computeFunnel` как `platformCutoffs`
 *      обязан вызывающий: `scripts/funnel-page.mjs` строит её из frontmatter
 *      `wiki/jobsearch/platforms/*.md` (поле `since`) и передаёт вместе с
 *      `asOf`/`ghostAfterDays`/`connectedSources`. Но `renderFunnelPage` остаётся ЧИСТОЙ
 *      функцией над готовым `FunnelReport`, а сам отчёт не хранит, был ли `platformCutoffs`
 *      передан и какой — поэтому раздел «Ограничения» ниже говорит об этом условно
 *      («ТОЛЬКО если вызывающий передал карту»), а не как о свершившемся факте: тест на
 *      пустых `opts` без карты обязан оставаться валидным сценарием.
 *
...
function stageTable(report: FunnelReport): string {
	...
	return [
		'| Стадия | Сейчас на стадии | Когда-либо достигали |',
		'| --- | --- | --- |',
		...rows,
	].join('\n');
}
...
/**
 * segmentTable — один разрез: значение, знаменатель, доля с любой реакцией и доля офферов.
 *
 * Колонка называется «Любая реакция», НЕ «Ответили» (MAJOR-находка ревью): в `funnel.ts`
 * `SegmentStat.replied` считает ЛЮБОЕ внешнее событие (`state.lastExternalAt` — включая
 * автоотказ), а не конкретно стадию `replied`. Если бы колонка называлась «Ответили», её
 * цифра читалась бы как то же самое, что стадия «Ответили» в «По стадиям» и конверсия
 * `applied→replied` в «Конверсии» — а это разные числа, и это систематически выше:
 * пояснение стоит один раз перед первой таблицей разреза (`explainSegmentColumn`).
 */
function segmentTable(buckets: Record<string, SegmentStat>): string {
	...
	return [
		'| Значение | Откликов | Любая реакция | Офферы |',
		'| --- | --- | --- | --- |',
		...rows,
	].join('\n');
}
...
/**
 * explainSegmentColumn — пояснение к колонке «Любая реакция», один раз перед первой
 * таблицей разреза (см. doc-комментарий `segmentTable`). Обе величины из этого абзаца
 * печатаются на странице под разными именами: «Ответили» — в «По стадиям» и в конверсии
 * ${c('applied→replied')} выше, «Любая реакция» — здесь.
 */
function explainSegmentColumn(): string {
	return (
		`> **«Любая реакция»** — это НЕ стадия «Ответили» и НЕ конверсия ${c('applied→replied')} ` +
		'из раздела «Конверсии» выше. Здесь считается ЛЮБОЕ внешнее событие после подачи, ' +
		'включая автоотказ, поэтому число тут закономерно выше: разница — это в основном ' +
		'отклики, ушедшие сразу в отказ, а не в переписку.'
	);
}
...
- Точка отсечения площадки (${c('since')}, [PRD] US 27) применяется в разрезе «По площадке»
  ниже, ТОЛЬКО если вызывающий передал карту ${c('platformCutoffs')} в ${c('computeFunnel')}
  ([ADR-0033]) — сама эта функция карту из реестра не строит, она получает уже готовый
  отчёт. Если её никто не передал, разрез посчитан по всей истории площадки, включая
  отклики до отсечения. Конкретные площадки и даты — в реестре
  [${c('wiki/jobsearch/platforms/')}](platforms/) (поле ${c('since')}), не здесь: эта
  страница про них не знает, только про сам факт, что отсечение может быть.
- «По стадиям» и «Конверсии» выше карты отсечения не видят никогда — это осознанное
  решение PRD (US 27: «в общую входит»), а не то же ограничение, что у разреза выше.
...
Та же колонка «Любая реакция», то же пояснение — см. раздел выше. Отсечение по площадке
(${c('since')}) применено, только если вызывающий его передал — см. «Ограничения».

${segmentTable(report.breakdowns.platform)}
