...
> ### ✅ Статус: Phase 7 ПРОЙДЕНА (2026-06-09)
> Все тракты закрыты на живой системе. **Capture-постура** ([ADR-0015](../docs/adr/0015-capture-write-path-permission-posture.md), решено через `/grill-with-docs`): движок пишет файлы вики под `acceptEdits` (без shell/сети, `.claude/settings.json` в content-репо), а **коммитит доверенный код** — мост per-turn (реактив) / `run_sweep.sh` (плановое), НЕ движок (§11 соблюдён). Входящий текст маскируется `failClosedSanitize()` до движка (§2). Из живого E2E-фидбэка: sweep-интервал 30→**5 мин** (отзывчивые напоминания); конверт-промпт моста получил runtime-humility (движок не врёт про launchd/планировщик). Транспорт — long polling ([ADR-0014](../docs/adr/0014-telegram-transport-long-polling.md)), порт **8787**. Дальше — Phase 8 (ингест).

- [x] **7.1 Реактив: вопрос → ответ.** В чате с ботом напиши:
  ```
  привет, ты живой?
  ```
...
- [x] **7.2 Реактив: непрерывность диалога** (`--resume` работает). Следом:
  ```
...
- [x] **7.3 Capture: заметка → запись в вику.** Кинь факт (без вопросительного знака — это «capture»-жест):
  ```
...
- [x] **7.4 Безопасность: чужой чат дропается** (single-user allow-list, [ADR-0009](../docs/adr/0009-tos-safe-engine-access.md)). (Опционально, если есть второй Telegram-аккаунт.) Напиши боту с другого аккаунта — ожидаем **молчание** (мост дропает апдейт, чей `chat_id` ≠ `TELEGRAM_OWNER_CHAT_ID`, и логирует это без ответа).

- [x] **7.5 Плановое: вручную дёрнуть sweep** (не ждать расписания). Запусти агент немедленно:
  ```bash
...
- [x] **7.6 (Контроль лимитов/`$0`).** Проверь живой запас лимитов подписки:
  ```bash
