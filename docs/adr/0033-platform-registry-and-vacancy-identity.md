...
**Контекст.** Площадка сегодня размазана по трём полям и двум репозиториям. `company_source` (168 `manual`) не различает ~25 разных ATS; `submission_channel` в 409 из 436 записей `direct`; `vacancy_ref` хранит то URL, то `tag:id`, причём ashby/greenhouse/lever/icims встречаются обоими способами. У hh есть код-адаптер на 29KB, у LinkedIn механика живёт прозой в скилле на 150 строк и в `references/platforms.md`. Добавление hh потребовало ADR, модуль, правку enum и правку двух SKILL.md. Дубль enum в `jobsearch-intent.ts` без `hh` пережил [ADR-0031](0031-hh-channel.md) §1. 34 записи с `company_source: linkedin` молча выпадают из воронки, потому что значения нет в словаре, хотя семантически оно верно (живой поиск в LinkedIn, а не файл экспорта). Одна компания из воронки владельца живёт под двумя id. Владелец хочет добавлять площадки так, чтобы хранилище их «запоминало».
...
`company.id = slug(site_domain)`; `opportunity.id = <external_id>`; `application.id = <external_id>` при известном внешнем id, иначе прежний id; повторный отклик на ту же вакансию получает суффикс `-r2`. События ссылаются на `application.id`.

> **Уточнение решения (при реализации).** Черновая формулировка этого пункта была `<platform>-<external_id>` — второй, разделённый префикс площадки. Это противоречило §2 того же ADR и [ADR-0031](0031-hh-channel.md) §4: `external_id` уже несёт буквенный префикс площадки без разделителя (`hh136857307`), поэтому `<platform>-<external_id>` дал бы `hh-hh136857307`. Реализация (`src/ingest/jobsearch/migrate-platforms.ts`, комментарий у сборки нового id) и мигрированные данные идут по `id = <external_id>` напрямую — так же, как уже было сделано для hh в [ADR-0031](0031-hh-channel.md). Формула выше приведена к этому; исторический факт («в первой формулировке решения был лишний префикс») не стирается, а фиксируется здесь как уточнение, не как переписывание.
