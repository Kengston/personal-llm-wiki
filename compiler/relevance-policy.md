...
> Единственный tunable источник правил фильтра ([ADR-0011](../docs/adr/0011-relevance-sensitivity-filter.md)). `src/ingest/classifier.ts` и compile-шаг читают **JSON-блок** ниже (встроенный `JSON.parse`, без отдельной YAML-зависимости — идиоматичный TS по [ADR-0012](../docs/adr/0012-language-typescript-port.md)). Проза вокруг объясняет каждую ручку; каждое изменение политики — обычный git-diff, ревьюится и откатывается.
>
> **Граница двух репо ([ADR-0003]).** Здесь — ТОЛЬКО публичная схема + безопасные дефолты. **Персональные значения** (имена как триггеры, приватные домены, расширенные лексиконы) кладёшь в ПРИВАТНЫЙ репо `llm-wiki-content/.filter-policy.local.json` — он мёрджится поверх этого в рантайме. Кастом-лексикон NSFW-терминов в публичном репо = утечка, которую `src/scheduler/lint-public.ts` не ловит, поэтому он **только** приватно.
...
- **Неэндорсмент-подача `knowledge/` ([ADR-0020](../docs/adr/0020-knowledge-library-fidelity-and-risk-tagging.md)).** Внешняя методология (категория-сосед `knowledge/`, [ADR-0019](../docs/adr/0019-knowledge-library-external-methodology.md)) — **не** про чувствительность/важность (это не sensitive в смысле [ADR-0011]; серый приём ≠ карантин). Движок **подаёт knowledge-claims с атрибуцией к источнику + caveat риска** (`risk: grey|legal|stale`), **никогда** как персональную рекомендацию «делай так»; юридически/налогово чувствительное несёт пометку «справка, не персональная консультация». Метод источника хранится с полной точностью — фильтр/карантин его **не** трогает. Агентность — за владельцем.
