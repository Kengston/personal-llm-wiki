...
- **База знаний (knowledge-library)** — категория `knowledge/<topic>/` в приватном контент-репо для **внешней** source-attributed методологии (чужое знание: гайды, курсы, книги), компилируемой по Карпатому (свой `index.md`, кросс-ссылки, **помеченные противоречия между источниками**, compile-once). Сосед `wiki/`, **не** раздел внутри неё. _Не путать с_ `wiki/` (знание **про владельца** — [ADR-0010](docs/adr/0010-wiki-content-model.md)) и `raw/` (иммутабельный сырой источник). Решение — [ADR-0019](docs/adr/0019-knowledge-library-external-methodology.md). _Avoid_: «справочник», «база» (это не SQL/вектор — [ADR-0002](docs/adr/0002-no-embedder-pure-karpathy.md)).
- **Пакет знаний (knowledge-package)** — портативная форма авторства базы знаний: самостоятельный репо с ingest-friendly структурой + манифест маппинга, обезличенный, пригоден для ингеста в **несколько** LLM-вик. Персонализация (роадмап/трекер/прогресс) в пакет **не** входит — она в личном слое каждого инстанса ([ADR-0019](docs/adr/0019-knowledge-library-external-methodology.md)).
- **Финансовая цель (finance-goal)** — первоклассная страница `type: finance-goal`, чей **прогресс ВЫЧИСЛЯЕТСЯ из леджера** (число: баланс / сумма трат / остаток долга), а не отмечается вручную. Виды: накопить к таргету · потолок трат по категории · гашение долга · рост net-worth. _Не путать с_ `growth` (качественная, самоотмечаемая цель развития; финансовую отличает детерминированное измерение из данных).
- **Доход (income)** — входящие потоки (зарплата / выручка / поступления), классифицированные в леджере; **descriptive-знание** о владельце (что и сколько получает), а НЕ цель. _Avoid_: путать «знать доход» с «таргетом по доходу» (последнее — finance-goal).
- **Леджер (ledger)** — структурный, append-only финансовый журнал в приватном репо (`raw/finance/`): счета, **снапшоты баланса** (строка на счёт на момент синка), транзакции, кредиты, курсы валют ([ADR-0018](docs/adr/0018-finance-module.md)). Источник истины для финансов; в `wiki/` попадают только огрублённые сводки. _Avoid_: «база»/«БД» (это не SQL и не вектор — [ADR-0002](docs/adr/0002-no-embedder-pure-karpathy.md)).
- **Мультивалютность** — каждая сумма хранится в НАТИВНОЙ валюте (ISO-4217 + крипто-символы), без захардкоженного списка валют (новые работают без правки кода); `fx_rate` фиксируется на момент синка для исторической точности. Роллапы показываются в настраиваемом НАБОРЕ валют (RUB/USD/GEL/…), а не в одной «базовой». _Avoid_: «базовая валюта» — единой базы нет, есть набор отображаемых; finance-goal деноминирована в своей валюте.
- **Наличные (cash)** — финансовый счёт `kind: cash` без API и уведомлений: остаток вводится вручную по проактивному запросу (periodic snapshot), мультивалютный как и всё (напр. VND, ₽). Дельта между снапшотами = чистый кэш-поток (без итемизации трат).
