...
- **Персона бота** — голос/роль, которым «Второй мозг» говорит с владельцем (на «ты», прячет внутрянку, не зовёт себя «движком»; регистр-aware). Настраиваемый слой моста ([ADR-0016](docs/adr/0016-bot-persona-configurable-system-prompt.md), системный промпт `--append-system-prompt`); контент персоны — **личный** (приватный `persona.md`), фреймворк несёт generic-дефолт + `persona.example.md`. _Не путать_ с контрактом хранителя (`compiler/rules.md` — что делать с вики, не как говорить).
