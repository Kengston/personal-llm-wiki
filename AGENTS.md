...
**Портируемость моста ([ADR-0008](docs/adr/0008-engine-claude-native.md)).** Весь мост знает движок только через абстракцию `Engine` (`src/bridge/engine.ts`); дефолт — `ClaudeEngine`. **`GrokEngine`** и **`CodexEngine`** — готовые **отложенные адаптеры-слоты** (опционально, добавляются конфигом без переписывания):
...
> **Стек — TypeScript** (Node 24, strict ESM), порт с Python ([ADR-0012](docs/adr/0012-language-typescript-port.md)): Fastify + pino + zod + dotenv-flow + `node:sqlite` + luxon + rrule + vitest, пакетный менеджер `pnpm`. Зеркалит house-style владельца (`abcage-mcp-hub`). Исходники — под `src/`, собираются в `dist/` командой `pnpm build` (`tsc`). Язык **не несущий** — инварианты держит ПОВЕДЕНИЕ, а не язык/число зависимостей ([ADR-0012](docs/adr/0012-language-typescript-port.md)).
...
├── package.json          # pnpm-скрипты (build/dev/test/lint:public/typecheck) + зависимости (Fastify/pino/zod/luxon/rrule)
├── compiler/
...
> Отложенные коннекторы (`vk` / `whatsapp` / `youtube` / `x` / `codebase`-graphify) и routine ревью-карантина в TS-порт пока **не перенесены** — это **запланированные** будущие модули, не существующие файлы ([ADR-0012](docs/adr/0012-language-typescript-port.md)). В v1 реальны только два коннектора: `src/ingest/llm-chat.ts` и `src/ingest/telegram-export.ts`.
...
2. **Sanitizer — в write-path, fail-closed.** Любой текст из источника проходит `sanitizeText(text)` (`src/ingest/sanitizer.ts`) **ДО** записи в `raw/`, и весь публичный репо проверяется `scanSecrets(text)` **ДО** коммита. Маскер упал / нашёл секрет — **запись/коммит отменяется** (обязанности — [compiler/rules.md §2](compiler/rules.md)).
...
4. **Без embedder / vec-DB** ([ADR-0002](docs/adr/0002-no-embedder-pure-karpathy.md)). Семантическое ранжирование делает сам движок, читая `index.md` → нужные страницы. Маленькая SQLite (`chat→session` в `bridge` через встроенный `node:sqlite` за интерфейсом `SessionStore`, опц. даты reminders) — это **не** нарушение (ADR-0002 запрещает векторный индекс для ВИКИ, не любую БД).
5. **Watermark на источник** ([src/ingest/watermark.ts](src/ingest/)) двигается **только после успешной записи** → повторный ингест идемпотентен.
...
7. **Публичное ≠ приватное.** В этот репо личные данные не попадают; страж — [src/scheduler/lint-public.ts](src/scheduler/) (exit≠0 при находке PII/секрета; `pnpm lint:public` после сборки).
...
1. Коннектор из `src/ingest/` читает экспорт → каждый фрагмент через **`sanitizeText()`** → пишет markdown в `raw/<source>/...` с provenance-frontmatter (`source`, `exported_at`, `range`, watermark-cursor). Sanitizer fail-closed (инвариант 2). Код-источник (`codebase`-graphify, **отложенный** коннектор — [ADR-0012](docs/adr/0012-language-typescript-port.md)) — отдельным треком, минует sanitizer (код — не PII).
...
1. Прогнать текст сообщения через **`sanitizeText()`** перед любой записью (инвариант 2) — даже из Telegram сюда может попасть чужой токен/телефон.
...
- **Публичный репо** ([src/scheduler/lint-public.ts](src/scheduler/), запуск `pnpm lint:public` после `pnpm build`): рекурсивный скан этого репо на PII/секреты через `scanSecrets` из `src/ingest/sanitizer.ts`; exit≠0 при находке (хук/CI перед коммитом). Это страж границы двух репо (инвариант 7).
